...
def load_model():
    try:
        model = tf.keras.models.load_model('models/drowsiness_model.h5', compile=False)
        return model
...
mode = st.radio("Choose Input Mode", ["🖼️ Image Upload (Mode A)", "📷 Live Camera (Mode B)"])

# ----------------------------------------------------------
# Mode A: Static Image Upload
# ----------------------------------------------------------

if mode == "🖼️ Image Upload (Mode A)":
...
# ----------------------------------------------------------
# Mode B: Live Camera - Enhanced Stability
# ----------------------------------------------------------
    ...
    recent_preds = deque(maxlen=15)  # last 15 frames (~1.5 seconds)
